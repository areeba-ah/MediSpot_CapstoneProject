...
public class RegisteredUserResultPage extends AppCompatActivity implements drugResultAdapter.onClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_registered_user_result_page);
        ...
        if("SavedDrug".equals(from)){
            ...
            for (String a : arrOfStr){byText(a);}
        }
    }
    ...
    private void byText(String value){
        check.add("testing");
        db.collection("Drug_List").orderBy("DrugName").startAt(value).endAt(value + "\uf8ff").get()
                .addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    @Override
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        if (!queryDocumentSnapshots.isEmpty()) {
